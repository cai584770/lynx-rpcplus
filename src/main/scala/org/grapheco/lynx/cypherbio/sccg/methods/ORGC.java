...
public class ORGC {
		...
		while (true) {
			...
			if (!match) {
				for (int k = kmer_location[key]; k != -1; k = next_kmer[k]) {
					...
					while (ref_idx < reference.length() && tar_idx < target.length()
							&& (reference.substring(ref_idx, ref_idx + 1)
									.equals(target.substring(tar_idx, tar_idx + 1)))) {
						ref_idx++;
						tar_idx++;
						increment++;
					}
	...
	public static List<Position> format_matches(String sequence) throws IOException {
		List<Position> list = new ArrayList<Position>();
		boolean successive = false;
		int start = 0, end = 0;
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		File reference_chrName = new File(args[0]);
		File target_chrName = new File(args[1]);

		ArrayList<String> fileNames;
		String[] chrName = null;
		if (target_chrName.exists() && target_chrName.isDirectory()) {
			File[] files = target_chrName.listFiles();
			...
			System.out.println("The folder does not exist or the specified path is not a folder!");
		}

		// final file name
		String referenceFloderName = args[0].substring(args[0].lastIndexOf('/') + 1);
		String targetFloderName = args[1].substring(args[1].lastIndexOf('/') + 1);
		String result = targetFloderName + "_ref_" + referenceFloderName;

		String reference_genome;
		String target_genome;
		String final_folder = args[2] + "//" + result;// output temp file folder
		Runtime.getRuntime().exec("mkdir " + final_folder);
		String final_file;
		int controuble;
		boolean is_con;
		...
		for (i = 0; i < chrName.length; i++) {
			...
			reference_genome = args[0] + "/" + chrName[i]; // reference file .fa path
			target_genome = args[1] + "/" + chrName[i]; // target file .fa path
			final_file = final_folder + "/" + chrName[i];
			...
			while (local) {
				...
				while (true) {
					...
					ORGC utilities = new ORGC();
					int kmerlength = ORGC.kmer_length;
		...
		use7zip(final_folder);
		Runtime.getRuntime().exec("rm -rf " + final_folder);
	...
	public static void compress(String reference_path, String target_path, String output_path)
			throws IOException, InterruptedException {
		String[] args = new String[] { reference_path, target_path, output_path };
		...
		// final file name
		String referenceFloderName = reference_path.substring(reference_path.lastIndexOf('/') + 1);
		String targetFloderName = target_path.substring(target_path.lastIndexOf('/') + 1);
		String result = targetFloderName + "_ref_" + referenceFloderName;
